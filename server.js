...
function removeBot(room, botId) {
  ...
  if (bot && bot.interval) clearInterval(bot.interval);
...
io.on('connection', (socket) => {
  ...
  socket.on('createRoom', (data = {}) => {
    ...
    for (let i = 0; i < botCount; i++) {
      ...
      bots.push({ id: bot.id, name: bot.name, color: bot.color });
    }

    socket.emit('roomCreated', { code, playerId: socket.id, bots });
    startRound(room);
  });
    ...
    for (const [code, room] of rooms) {
      if (room.players.has(socket.id)) {
        ...
        io.to(code).emit('playerJoined', {
          ...
          score: bot.score, color: bot.color, direction: bot.direction, isBot: true
        });
        break;
      }
    }
  ...
  socket.on('joinRoom', (data) => {
    ...
    if (room.players.size >= 8) { socket.emit('error', 'Sala cheia (máx 8 jogadores)'); return; }
    ...
    const playersData = Array.from(room.players.values()).map(p => ({
      id: p.id, name: p.name || p.id.slice(0, 6),
      x: p.x, y: p.y, score: p.score, color: p.color, direction: p.direction, isBot: p.isBot || false
    }));

    socket.emit('joinedRoom', {
      code, playerId: socket.id, players: playersData,
      apples: room.apples,
    ...
    socket.to(code).emit('playerJoined', {
      ...
      score: player.score, color: player.color, direction: player.direction, isBot: false
    });
  });
  ...
  socket.on('direction', (dir) => {
    for (const [code, room] of rooms) {
      ...
      if (p && !p.isBot) {
        if (!room.gameActive) return;
        const opposites = { up: 'down', down: 'up', left: 'right', right: 'left' };
        if (opposites[dir] !== p.direction) p.direction = dir;
        break;
      }
    }
  });
    ...
    for (const [code, room] of rooms) {
      ...
      if (p) {
        io.to(code).emit('chatMessage', {
          id: socket.id, name: p.name || p.id.slice(0, 6),
          color: p.color, msg, isBot: false
        });
        break;
      }
    }
...
server.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`\n💡 Para jogar pela internet:`);
  console.log(`   1. Use ngrok: ngrok http ${PORT}`);
  console.log(`   2. Ou configure redirecionamento de porta no roteador\n`);
});
